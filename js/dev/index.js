import { d as getDigFormat, u as uniqArray, g as gsapWithCSS } from "./app.min.js";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const STORAGE_KEY = "flsPreloaderShown";
  ...
  function setProgress(value) {
    ...
    if (lineEl) lineEl.style.width = `${value}%`;
  }
  function imageLoaded() {
    loadedCount++;
    progress = Math.round(100 / images.length * loadedCount);
    const id = setInterval(() => {
      if (counter >= progress) {
        clearInterval(id);
        if (counter >= 100) finishPreloader();
      } else {
        setProgress(++counter);
      }
    }, 15);
  ...
  function animateBars() {
    return new Promise((resolve) => {
      function step() {
        if (increment >= innerBars.length) return resolve();
        for (let i = 0; i < 2; i++) {
          if (innerBars[i + increment]) {
            gsapWithCSS.to(innerBars[i + increment], {
              width: "100%",
              duration: 0.3,
              ease: "none",
              delay: 0.3
